...
HINT: One of these approaches uses ROW_NUMBER() and one uses DENSE_RANK(). */
 SELECT * 
 ,ROW_NUMBER() OVER(PARTITION BY customer_id ORDER BY market_date) AS purchase_rank
...
ve.vendor_name,
pr.product_name,
SUM(5 * vi.original_price) AS revenue
FROM vendor_inventory vi
JOIN vendor ve ON vi.vendor_id = ve.vendor_id
JOIN product pr ON vi.product_id = pr.product_id
CROSS JOIN customer c
GROUP BY ve.vendor_name, pr.product_name
ORDER BY ve.vendor_name, pr.product_name;
...
VALUES(25,'Apple Pie','3 lb',9,'unit');
...
UPDATE product_units AS po
SET current_quantity = COALESCE(
( SELECT vi.quantity
FROM vendor_inventory vi
WHERE vi.product_id = po.product_id
